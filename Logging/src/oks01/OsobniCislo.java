...
public class OsobniCislo implements Comparable<OsobniCislo> {
  public static final Logger osobniCisloLogger = LogManager.getLogger();
  ...
  public OsobniCislo(String radkaZadani) {
    ...
    osobniCisloLogger.trace(vysledek);
  }
  ...
  public void generujOsobniCislo(String poradoveCislo) {    
    ...
    osobniCisloLogger.debug(vysledek);
  }
  ...
  private void zpracujPrijmeni(String prijmeni) {
    if (prijmeni != null) {
      ...
      osobniCisloLogger.trace("prijmeni: " + this.prijmeni);
    }
    else {
  ...
  private void zpracujJmeno(String jmeno) {
    if (jmeno != null) {
      ...
      osobniCisloLogger.trace("jmeno: " + this.jmeno);
    }
    else {
  ...
  private void zpracujRokNastupu(String rokNastupu) {
    if (rokNastupu == null) {
      chybnyRokNastupu();
      osobniCisloLogger.trace("neni uveden rok nastupu");
      return;
    }
    if (rokNastupu.length() != 4) {
      // není čtyřmístný
      chybnyRokNastupu();
      osobniCisloLogger.trace("rok nastupu neni ctyrciferny: " + rokNastupu);
      return;
    }
    ...
    osobniCisloLogger.trace("rok nastupu: " + this.rokNastupu);
  }
  ...
  public void zpracujFakulta(String fakulta) {
    for (String[] fakulty : Konstanty.PLATNE_FAKULTY) {
      if (fakulty[0].equals(fakulta) == true) {
        ...
        osobniCisloLogger.trace("fakulta: " + this.fakulta);
        return;
      }     
    }
  ...
  private void zpracujTypStudia(String typStudia) {
    for (TypStudia typ : TypStudia.values()) {
      if (typ.getZkratka().equals(typStudia) == true) {
        this.typStudia = typ;
        osobniCisloLogger.trace("typ studia: " + this.typStudia.getZkratka());
        return;
      }     
    }
  ...
  private void zpracujFormaStudia(String formaStudia) {
    for (String forma : Konstanty.PLATNE_FORMY_STUDIA) {
      if (forma.equals(formaStudia) == true) {
        ...
        osobniCisloLogger.trace("forma studia: " + this.formaStudia);
        return;
      }     
    }
  ...
  private void zpracujNepovinne(String nepovinne) {
    if (nepovinne != null) {
      ...
      osobniCisloLogger.trace("nepovinne: " + this.nepovinne);
    }
    else {
      this.nepovinne = "";
    }
